...
class D2RL_SAC(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args,layer_num=4):
        super(D2RL_SAC,self).__init__()
        self.args = args
        self.actor = D2RL_Actor(self.args.layer_num, state_dim, action_dim, self.args.hidden_dim, \
                           self.args.activation_function, self.args.last_activation, self.args.trainable_std)

        self.q_1 = D2RL_Critic(layer_num, state_dim+action_dim, 1, self.args.hidden_dim, self.args.activation_function,self.args.last_activation)
        self.q_2 = D2RL_Critic(layer_num, state_dim+action_dim, 1, self.args.hidden_dim, self.args.activation_function,self.args.last_activation)
        
        self.target_q_1 = D2RL_Critic(layer_num, state_dim+action_dim, 1, self.args.hidden_dim, self.args.activation_function, self.args.last_activation)
        self.target_q_2 = D2RL_Critic(layer_num, state_dim+action_dim, 1, self.args.hidden_dim, self.args.activation_function, self.args.last_activation)
