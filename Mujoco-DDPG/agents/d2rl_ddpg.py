...
class D2RL_DDPG(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args, noise,layer_num=4):
        super(D2RL_DDPG,self).__init__()
        self.device = device
        self.writer = writer
        
        self.args = args
        self.actor = D2RL_Actor(layer_num, state_dim, action_dim, self.args.hidden_dim, \
                           self.args.activation_function, self.args.last_activation, self.args.trainable_std)
        
        self.target_actor = D2RL_Actor(layer_num, state_dim, action_dim, self.args.hidden_dim, \
                           self.args.activation_function, self.args.last_activation, self.args.trainable_std)
        
        self.q = D2RL_Critic(layer_num, state_dim+action_dim, 1, self.args.hidden_dim, self.args.activation_function, None)
        
        self.target_q = D2RL_Critic(layer_num, state_dim+action_dim, 1, self.args.hidden_dim, self.args.activation_function, None)
