...
class D2RL_Network(NetworkBase):
    def __init__(self, layer_num, input_dim, output_dim, hidden_dim, activation_function = torch.relu,last_activation = None):
        super(D2RL_Network, self).__init__()
        self.activation = activation_function
        self.last_activation = last_activation
        layers_unit = [input_dim]+ [hidden_dim]*(layer_num-1)
        self.initial_layer=nn.Linear(layers_unit[0],layers_unit[1])
        layers = ([nn.Linear(layers_unit[idx]+input_dim,layers_unit[idx+1]) for idx in range(1,len(layers_unit)-1)])
    ...
    def _forward(self, x):
        ...
        for layer in self.layers:
            x=torch.cat([x,initial_info],dim=-1)
            x=self.activation(layer(x))
